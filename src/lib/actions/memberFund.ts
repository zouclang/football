...
export async function deleteMemberFundTransaction(id: string) {
    try {
        ...
        // 【联动业务撤销】：
        // 1. 如果是“赞助费转账”记录（意味着当时会员费不足，由赞助费垫付），删除时应同步删除赞助费端的支出记录
        if (tx.type === 'INCOME' && tx.description === '赞助费转账' && tx.matchId) {
            await prisma.teamFundTransaction.deleteMany({
                where: {
                    category: "会员费兜底",
                    description: { contains: `[Ref:${tx.matchId}]` }
                }
            })
        }

        // 2. 如果是收款单且删除了，看是否需要剥夺会籍？
